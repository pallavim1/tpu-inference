...
const RPS_LEVELS   = [40, 50, 60, 70, 80, 90, 100, 120, 140, 160, 180, 200];
...
function makeFixedPayloadScenario(exec, rps, vus, sizeBytes, startSec) {
  const scaledPreVUs = Math.max(vus, rps * 2);
  const scaledMaxVUs = Math.max(vus * 4, rps * 8);
  return {
    executor: 'constant-arrival-rate',
    exec,
    rate: rps,
...
function payloadSizeScenarios() {
  ...
  // prompt-c2: 1 K, 2 K, 5 K, 7 K
  if (ep === 'prompt_c2' || ep === 'both') {
    const promptSizes = [1024, 2048, 5120, 7168];
    promptSizes.forEach((sz) => {
      s[`prompt_c2_${sz}b`] = makeFixedPayloadScenario(
        promptExec, PAYLOAD_RPS, PAYLOAD_VUS, sz, offset * PAYLOAD_STAGE_S,
      );
      offset++;
    });
  }
  ...
  // mcp-c2: 100, 200, 400
  if (ep === 'mcp_c2' || ep === 'both') {
    const mcpSizes = [100, 200, 400];
    mcpSizes.forEach((sz) => {
      s[`mcp_c2_${sz}b`] = makeFixedPayloadScenario(
        mcpExec, PAYLOAD_RPS, PAYLOAD_VUS, sz, offset * PAYLOAD_STAGE_S,
      );
      offset++;
    });
  }
