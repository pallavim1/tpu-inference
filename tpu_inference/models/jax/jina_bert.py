...
"""JAX/Flax NNX implementation of JinaBERT (`JinaBertForMaskedLM`) with
TPU v6e (Trillium) 4-Layer Fused FP32 Megakernel (`jina_v6e_4layer_megakernel`).
"""
...
def get_alibi_slopes(num_heads: int) -> List[float]:
    """Compute per-head ALiBi slopes matching Jina's `get_slopes`."""
...
def _set_weight_loader(
    ...
    """Attach a custom `weight_loader` to `param` that reshapes and permutes."""

    def loader(dst: nnx.Param, loaded: jax.Array) -> None:
        arr = loaded
        if reshape_dims is not None:
            arr = jnp.reshape(arr, reshape_dims)
        if permute_dims is not None:
            arr = jnp.transpose(arr, permute_dims)
        assign_and_shard_param(dst, arr, name)

    param.set_metadata("weight_loader", loader)


class JinaBertEmbeddings(JaxModule):
    """Word + token_type embeddings followed by LayerNorm."""
    ...
    def __init__(self, config, dtype: jnp.dtype, rng: nnx.Rngs):
        self.word_embeddings = JaxEmbed(
            ...
            rngs=rng,
            prefix="embeddings.word_embeddings",
        )
        self.token_type_embeddings = JaxEmbed(
            ...
            rngs=rng,
            prefix="embeddings.token_type_embeddings",
        )
    ...
    def __call__(self, input_ids: jax.Array) -> jax.Array:
        ...
        type_emb = self.token_type_embeddings.embedding.value[0]
        return self.LayerNorm(word_emb + type_emb)


class JinaBertSelfAttention(JaxModule):
    """Bidirectional multi-head self-attention with symmetric ALiBi."""
    ...
    def __init__(
        ...
        def qkv(name):
            ...
            _set_weight_loader(
                ...
                reshape_dims=(self.num_heads, self.head_dim, self.hidden_size),
                permute_dims=(2, 0, 1),
            )
            _set_weight_loader(
                proj.bias,
...
class JinaBertEncoder(JaxModule):
    ...
    def build_megakernel_weights(self) -> None:
        """Packs and fuses all 4 layers' weights into contiguous Megakernel tensors."""
        w_qkv = jnp.stack(
            [
                jnp.stack(
                    [
        ...
        self.mega_w_qkv = nnx.Param(w_qkv)
        self.mega_b_qkv = nnx.Param(b_qkv)
        self.mega_w_o = nnx.Param(w_o)
        self.mega_b_o = nnx.Param(b_o)
        self.mega_ln1_scale = nnx.Param(ln1_scale)
        self.mega_ln1_bias = nnx.Param(ln1_bias)
        self.mega_w_gated = nnx.Param(w_gated)
        self.mega_w_down = nnx.Param(w_down)
        self.mega_b_down = nnx.Param(b_down)
        self.mega_ln2_scale = nnx.Param(ln2_scale)
        self.mega_ln2_bias = nnx.Param(ln2_bias)

    def __call__(
        self, x: jax.Array, attention_metadata: AttentionMetadata
    ) -> jax.Array:
        if hasattr(self, "mega_w_qkv"):
            w_qkv = self.mega_w_qkv.value
            b_qkv = self.mega_b_qkv.value
            w_o = self.mega_w_o.value
            b_o = self.mega_b_o.value
            ln1_scale = self.mega_ln1_scale.value
            ln1_bias = self.mega_ln1_bias.value
            w_gated = self.mega_w_gated.value
            w_down = self.mega_w_down.value
            b_down = self.mega_b_down.value
            ln2_scale = self.mega_ln2_scale.value
            ln2_bias = self.mega_ln2_bias.value
        else:
            w_qkv = jnp.stack(
                [
                    jnp.stack(
...
class JinaBertForMaskedLM(JaxModule, LoadableWithIterator):
    ...
    def load_weights(self, weights):
        """Loads HF checkpoint weights and packs them into the 4-Layer Fused Megakernel."""
        from tpu_inference.models.jax.utils.weight_utils import (
        ...
        result = loader.load_weights(_filtered(weights))
        # Pre-pack and fuse all 4 encoder layers' weights into contiguous Megakernel tensors
        self.model.encoder.build_megakernel_weights()
        return result
